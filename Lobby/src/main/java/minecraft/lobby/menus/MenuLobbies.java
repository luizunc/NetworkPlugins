...
import static minecraft.lobby.lobby.Lobby.listLobbies;
...
public final class MenuLobbies extends UpdatablePlayerMenu {
    ...
    private static final String CURRENT_LOBBY_ICON = "STAINED_GLASS:5 : 1 : nome>&aLobby %s : desc>&fJogadores: &7{players}/{maxplayers}\n \n{description}";
    private static final String OTHER_LOBBY_ICON = "STAINED_GLASS:0 : 1 : nome>&fLobby %s : desc>&fJogadores: &7{players}/{maxplayers}\n \n{description}";
    ...
    private boolean canConnectToLobby(Lobby lobby) {
        return !Main.getCurrentServerName().contentEquals(lobby.getServerName()) 
    ...
    private String createIconString(Lobby lobby) {
        String lobbyNumber = lobby.getServerName().replace("lobby", "");
        
        if (isCurrentLobby(lobby)) {
            return String.format(CURRENT_LOBBY_ICON, lobbyNumber);
        } else {
            return String.format(OTHER_LOBBY_ICON, lobbyNumber);
        }
    }
    ...
    private boolean isCurrentLobby(Lobby lobby) {
        return Main.getCurrentServerName().equals(lobby.getServerName());
    }
