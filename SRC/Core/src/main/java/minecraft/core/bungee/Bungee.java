...
public class Bungee extends Plugin {
  ...
  // Canais de plugin
  private static final String KCORE_CHANNEL = "kCore";
  ...
  public static void sendRole(ProxiedPlayer player, String sound) {
    ...
    try {
      ...
      player.getServer().sendData(KCORE_CHANNEL, out.toByteArray());
    } catch (Exception e) {
  ...
  public static void applyFake(ProxiedPlayer player, String fakeName, String role, String skin) {
    ...
    try {
      String kickMessage = getInstance().getConfig().getString("fake.kick-apply");
      if (kickMessage != null) {
        kickMessage = StringUtils.formatColors(kickMessage).replace("\\n", "\n");
  ...
  public static void removeFake(ProxiedPlayer player) {
    ...
    try {
      String kickMessage = getInstance().getConfig().getString("fake.kick-remove");
      if (kickMessage != null) {
        kickMessage = StringUtils.formatColors(kickMessage).replace("\\n", "\n");
        player.disconnect(TextComponent.fromLegacyText(kickMessage));
      }
  ...
  public static boolean isFakeRole(String roleName) {
    ...
    try {
      List<String> validRoles = getInstance().getConfig().getStringList("fake.role");
      return validRoles.stream().anyMatch(role -> role.equalsIgnoreCase(roleName));
    } catch (Exception e) {
      getInstance().getLogger().log(Level.WARNING, "Erro ao verificar role fake " + roleName, e);
      return false;
    }
  }
  ...
  public void onEnable() {
    try {
      ...
      // Registrar canal
      getProxy().registerChannel(KCORE_CHANNEL);
  ...
  public void saveDefaultConfig() {
    String[] configFiles = {"config", "utils"};
    ...
    for (String fileName : configFiles) {
      ...
      if (!file.exists()) {
        try {
          ...
          copyFile(getResourceAsStream(fileName + ".yml"), file);
        } catch (Exception e) {
          this.getLogger().log(Level.WARNING, "Erro ao criar arquivo " + fileName + ".yml", e);
        }
      }
      
      try {
        Configuration config = YamlConfiguration.getProvider(YamlConfiguration.class)
            .load(new InputStreamReader(new FileInputStream(file), StandardCharsets.UTF_8));
        
        if ("config".equals(fileName)) {
          this.config = config;
        } else if ("utils".equals(fileName)) {
          this.utils = config;
        }
      } catch (IOException e) {
        this.getLogger().log(Level.WARNING, "Erro ao carregar " + fileName + ".yml", e);
      }
    }
  }
    ...
    try {
      ...
      if (utils.get("fake.role") instanceof String) {
        ...
        YamlConfiguration.getProvider(YamlConfiguration.class)
            .save(utils, new File("plugins/kCore/utils.yml"));
      }
    } catch (IOException e) {
