...
import dev.slickcollections.kiwizin.murder.cmd.mm.*;
...
public class MurderCommand extends Commands {
  ...
  public MurderCommand() {
      ...
      if (Main.kMysteryBox) {
          this.commands.add(new MysteryBoxesCommand());
      }
  ...
  public void perform(CommandSender sender, String label, String[] args) {
      if (!sender.hasPermission("kmurder.cmd.murder")) {
          sender.sendMessage("§6kMurder §bv" + Main.getInstance().getDescription().getVersion() + " §7Criado por §6Kiwizin§7.");
          return;
      }
  ...
  private void sendHelp(CommandSender sender, int page) {
      ...
      for (int index = 0; index < commands.size(); index++) {
          ...
          pages.get(currentPage).append("§6/mm ").append(commands.get(index).getUsage()).append(" §f- §7").append(commands.get(index).getDescription()).append("\n");
      }
