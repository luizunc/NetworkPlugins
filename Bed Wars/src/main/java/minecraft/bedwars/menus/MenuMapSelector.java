...
import minecraft.core.core.libraries.menu.UpdatablePlayerPagedMenu;
import minecraft.core.core.player.Profile;
import minecraft.core.core.utils.BukkitUtils;
import minecraft.core.core.utils.TimeUtils;
...
public class MenuMapSelector extends UpdatablePlayerPagedMenu {
              ...
              if (mapName != null && this.can) {
                ...
                for (BedWars game : this.games.get(mapName)) {
                  if (game.getState().canJoin() && game.getOnline() < game.getMaxPlayers()) {
                    ...
                    this.profile.setStats("bedwars", System.currentTimeMillis() + TimeUtils.getExpireIn(1), "lastmap");
  ...
  @Override
  public void update() {
    if (!this.player.hasPermission("bedwars.menu.selector") && profile.getStats("bedwars", "lastmap") >= System.currentTimeMillis()) {
      this.can = false;
    }
    ...
    this.removeSlotsWith(BukkitUtils.deserializeItemStack(Language.lobby$npc$play$menu$info$item.replace("{desc}", this.player.hasPermission("bedwars.menu.selector") ?
