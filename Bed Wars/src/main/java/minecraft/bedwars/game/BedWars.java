...
import minecraft.bedwars.game.object.BedWarsTask;
...
public class BedWars implements Game<BedWarsTeam> {
  ...
  public void destroyBed(BedWarsTeam team, Profile breaker) {
    ...
    team.listPlayers().forEach(t -> Profile.getProfile(t.getName()).addStats("bedwars", this.getMode().getStats() + "bedslosteds"));
  ...
  private void joinParty(Profile profile, boolean ignoreLeader) {
    ...
    this.broadcastMessage(Language.ingame$broadcast$join.replace("{player}", Rank.getColored(player.getName())).replace("{players}", String.valueOf(this.getOnline()))
      ...
      if (alive && this.state == GameState.EMJOGO) {
        ...
        for (Profile hitter : hitters) {
          if (!hitter.equals(killer) && hitter.playingGame() && hitter.getGame().equals(this) && !this.isSpectator(hitter.getPlayer())) {
            hitter.addStats("bedwars", this.getMode().getStats() + "assists");
            
            // Mensal
            hitter.addStats("bedwars", "monthlyassists");
          }
        }
        hitters.clear();
      }
  ...
  public void kill(Profile profile, Profile killer) {
    ...
    if (!team.bed()) {
      ...
      if (killer == null) {
        ...
        if (player.getLastDamageCause() == null || player.getLastDamageCause().getCause() != EntityDamageEvent.DamageCause.VOID) {
          KillEffect ke = killer.getAbstractContainer("bedwars", "selected", SelectedContainer.class).getSelected(CosmeticType.KILL_EFFECT, KillEffect.class);
          if (ke != null) {
      ...
      profile.addStats("bedwars", this.getMode().getStats() + "deaths");
    ...
    Bukkit.getScheduler().scheduleSyncDelayedTask(Main.getInstance(), () -> {
      if (player.isOnline()) {
        ...
        Bukkit.getScheduler().scheduleSyncDelayedTask(Main.getInstance(), () -> {
          if (player.isOnline()) {
            int coinsKill = (int) (this.getKills(player) * Language.options$coins$kills);
            int coinsBeds = (int) (this.beds.get(player.getName()) == null ? 0 : this.beds.get(player.getName()) * Language.options$coins$beds);
  ...
  public void stop(BedWarsTeam winners) {
    ...
    for (Player player : this.listPlayers(false)) {
      ...
      BedWarsTeam team = this.getTeam(player);
      if (team != null) {
        int coinsWin = (int) (team.equals(winners) ? Language.options$coins$wins : 0);
        int coinsKill = (int) (this.getKills(player) * Language.options$coins$kills);
        int coinsBeds = (int) (this.beds.get(player.getName()) == null ? 0 : this.beds.get(player.getName()) * Language.options$coins$beds);
      ...
      if (winners != null && winners.hasMember(player)) {
        profile.addCoins("bedwars", Language.options$coins$wins);
        profile.addStats("bedwars", this.getMode().getStats() + "wins");
