...
public class ShopkeeperNPC {
  ...
  public void spawn() {
    ...
    List<Profile> profiles = this.team.listPlayers().stream().map(player -> Profile.getProfile(player.getName()))
        .filter(profile -> profile.getAbstractContainer("bedwars", "selected", SelectedContainer.class).getSelected(CosmeticType.SHOPKEEPERSKIN, ShopkeeperSkin.class) != null)
        .collect(Collectors.toList());
    if (profiles.size() > 0) {
      skin = profiles.get(ThreadLocalRandom.current().nextInt(profiles.size())).getAbstractContainer("bedwars", "selected", SelectedContainer.class)
