...
import minecraft.bedwars.cosmetics.types.WoodTypes;
  ...
  public void addItem(ShopItem item) {
    ...
    for (ItemStack is : item.getContent().stream().filter(give::contains).collect(Collectors.toList())) {
      ...
      if (is.getType().name().contains("WOOD")) {
        WoodTypes dc = Profile.getProfile(player.getName()).getAbstractContainer("bedwars", "selected", SelectedContainer.class).getSelected(CosmeticType.WOOD_TYPE, WoodTypes.class);
        if (dc != null) {
