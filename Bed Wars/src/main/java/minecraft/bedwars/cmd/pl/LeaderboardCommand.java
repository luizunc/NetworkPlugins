...
public class LeaderboardCommand extends SubCommand {
  ...
  public void perform(Player player, String[] args) {
    if (args.length == 0) {
      player.sendMessage(" \n§eAjuda\n \n§6/pl leaderboard adicionar [id] [tipo] §f- §7Adicionar Leaderboard.\n§6/pl leaderboard remover [id] §f- §7Remover Leaderboard.\n ");
      return;
    }
    ...
    if (action.equalsIgnoreCase("adicionar")) {
      ...
      if (args.length <= 1) {
        player.sendMessage("§cUso: /pl leaderboard remover [id]");
        return;
      }
