package minecraft.bedwars.listeners.entity;
        ...
        if (proj.getShooter() instanceof Player) {
          ...
          if (proj instanceof Arrow) {
            ...
            Bukkit.getScheduler().scheduleSyncDelayedTask(Main.getInstance(), () -> finalDamager
                    .sendMessage(Language.ingame$messages$bow$hit.replace("{name}", Rank.getColored(player.getName())).replace("{hp}", StringUtils.formatNumber(player.getHealth()))),
                5L);
          }
        }
      }
