...
import minecraft.core.core.database.conversor.MongoDBConversor;
...
public class MongoDBDatabase extends Database {
  ...
  public MongoDBDatabase(String mongoURL) {
    ...
    this.tables =
        DataTable.listTables().stream().map(DataTable::getInfo).map(DataTableInfo::name).filter(name -> !name.equalsIgnoreCase("CoreProfile")).collect(Collectors.toList());
  ...
  public Map<String, Map<String, DataContainer>> load(String name) throws ProfileLoadException {
    ...
    for (DataTable table : DataTable.listTables()) {
      ...
      String prefix = table.getInfo().name().equalsIgnoreCase("coreprofile") ? "" : table.getInfo().name() + ".";
    ...
    if (document != null) {
      ...
      for (String key : document.keySet()) {
        ...
        tableMap.get("CoreProfile").put(key, new DataContainer(document.get(key)));
      }
    } else {
  ...
  private void save0(String name, Map<String, Map<String, DataContainer>> tableMap, boolean async) {
    ...
    for (DataTable table : DataTable.listTables()) {
      ...
      String prefix = table.getInfo().name().equalsIgnoreCase("coreprofile") ? "" : table.getInfo().name() + ".";
      if (table.getInfo().name().contains("SkyWars")) {
      ...
      // The Bridge removido do sistema
