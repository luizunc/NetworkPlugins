package minecraft.core.bungee.listener;
...
public class Listeners implements Listener {
  ...
  private static final Map<String, Object[]> PROPERTY_BACKUP = new HashMap<>();
  ...
  public void onPluginMessage(PluginMessageEvent evt) {
    if (evt.getSender() instanceof ServerConnection && evt.getReceiver() instanceof ProxiedPlayer) {
      if (evt.getTag().equalsIgnoreCase("Core")) {
        ...
        if (subChannel.equalsIgnoreCase("FAKE_SKIN")) {
          ...
          if (profile != null) {
            try {
              ...
              Object[] properties = PROPERTY_BACKUP.remove(player.getName().toLowerCase());
              if (properties != null) {
                profile.setProperties((net.md_5.bungee.protocol.data.Property[]) properties);
              }
            }
          }
        }
  ...
  private void modifyProperties(LoginResult profile, String[] data) {
    ...
    for (Object property : profile.getProperties() == null ? new ArrayList<Object>() : Arrays.asList(profile.getProperties())) {
      if (property.toString().contains("textures")) {
        continue;
      }
    ...
    // Criar nova propriedade de textura usando reflexão
    try {
      Class<?> propertyClass = Class.forName("net.md_5.bungee.connection.LoginResult$Property");
      Object newProperty = propertyClass.getConstructor(String.class, String.class, String.class).newInstance("textures", data[0], data[1]);
      properties.add(newProperty);
      profile.setProperties((net.md_5.bungee.protocol.data.Property[]) properties.toArray(new net.md_5.bungee.protocol.data.Property[0]));
    } catch (Exception e) {
